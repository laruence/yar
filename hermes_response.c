...
hermes_response_t * php_hermes_response_instance(TSRMLS_D) /* {{{ */ {
	hermes_response_t *response = ecalloc(sizeof(hermes_response_t), 1);

	return response;
} /* }}} */

int php_hermes_response_bind_request(hermes_response_t *response, hermes_request_t *request TSRMLS_DC) /* {{{ */ {
...
void php_hermes_response_set_exception(hermes_response_t *response, zval *ex TSRMLS_DC) /* {{{ */ {
	...
	response->status = HERMES_ERR_EXCEPTION;
    response->err = ret;
	zval_ptr_dtor(&ex);
} /* }}} */

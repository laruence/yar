...
int php_yar_listener_curl_listen(yar_listener_interface_t *self, char *address, uint len, zval *executor, char **err_msg  TSRMLS_DC) /* {{{ */ {
    ...
    if (!method || strncasecmp(method, "POST", 4)) {
            if (YAR_G(expose_info)) {
                    php_yar_listener_curl_info(executor TSRMLS_CC);
                    //RETURN_TRUE;
                    return 0;
            } else {
...
int php_yar_listener_curl_print_info(void *ptr, void *argument TSRMLS_DC) /* {{{ */ {
    zend_function *f = ptr;
...
response_no_output:
	php_yar_listener_curl_response(request, response TSRMLS_CC);
...
void php_yar_listener_curl_response(yar_request_t *request, yar_response_t *response TSRMLS_DC) /* {{{ */ {
	zval ret;
...
void php_yar_listener_curl_response_header(size_t content_lenth, void *packager_info TSRMLS_DC) /* {{{ */ {
...
void php_yar_listener_curl_close( yar_listener_interface_t *self TSRMLS_DC) /* {{{ */ {
    return;
} 
/* }}} */

yar_listener_interface_t * php_yar_listener_curl_init(TSRMLS_D) /* {{{ */ {
...
void php_yar_listener_curl_destroy(yar_listener_interface_t *self TSRMLS_DC) /* {{{ */ {
...
/* {{{ yar_listener_t yar_listener_curl
 */
yar_listener_t yar_listener_curl = {
	"curl",
	php_yar_listener_curl_init,
	php_yar_listener_curl_destroy,
	NULL
}; 
/* }}} */
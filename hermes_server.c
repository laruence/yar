...
zend_class_entry *hermes_server_ce;
...
#define HTML_MARKUP_HEADER  \
    ...
    "<html>\n" \
      ...
      "<head><title>Hermes RPC Framework</title>\n" \
		"<style type=\"text/css\">\n" \
...
static char * php_hermes_get_function_declaration(zend_function *fptr TSRMLS_DC) /* {{{ */ {
	char *offset, *buf;
...
static int php_hermes_print_info(void *ptr, void *argument TSRMLS_DC) /* {{{ */ {
    zend_function *f = ptr;
    zend_class_entry *ce = argument;

    if (f->common.fn_flags & ZEND_ACC_PUBLIC) {
        char *prototype = NULL;
		if ((prototype = php_hermes_get_function_declaration(f TSRMLS_CC))) {
...
static void php_hermes_server_response_header(size_t content_lenth, void *packager_info TSRMLS_CC) /* {{{ */ {
...
static void php_hermes_server_response(hermes_request_t *request, hermes_response_t *response TSRMLS_DC) /* {{{ */ {
	zval ret, *tmp;
	...
	INIT_ZVAL(ret);
	Z_TYPE(ret) = IS_ARRAY;
	ALLOC_HASHTABLE(Z_ARRVAL(ret));
	zend_hash_init(Z_ARRVAL(ret), 6, NULL, NULL, 0);

	add_assoc_long_ex(&ret, ZEND_STRS("i"), response->id);
	add_assoc_long_ex(&ret, ZEND_STRS("s"), response->status);
	if (response->out) {
		add_assoc_string_ex(&ret, ZEND_STRS("o"), response->out, 0);
	}
	if (response->retval) {
    ...
    if (!(payload_len = php_hermes_packager_pack(&ret, &payload, &err_msg TSRMLS_CC))) {
		zval_dtor(&ret);
		php_hermes_error(response, HERMES_ERR_PACKAGER, "%s", err_msg);
		efree(err_msg);
		return;
	}
	zval_dtor(&ret);

	php_hermes_protocol_render(&header, request->id, "PHP Hermes Server", NULL, payload_len, 0 TSRMLS_CC);
	php_hermes_debug_server("%ld: server response: packager '%s', len '%ld', content '%s'" TSRMLS_CC,
		   request->id, payload, payload_len - 8, payload + 8);

   php_hermes_server_response_header(sizeof(hermes_header_t) + payload_len, payload TSRMLS_CC);
	PHPWRITE((char *)&header, sizeof(hermes_header_t));
	if (payload_len) {
...
static void php_hermes_server_handle(zval *obj TSRMLS_DC) /* {{{ */ {
	char *payload, *err_msg;
	size_t payload_len;
	zend_bool bailout = 0;
	...
	hermes_request_t  *request = NULL;
	hermes_header_t *header;

	response = php_hermes_response_instance(TSRMLS_C);
	...
	if (!(header = php_hermes_protocol_parse(&payload, &payload_len, &err_msg))) {
        php_hermes_error(response, HERMES_ERR_PACKAGER, err_msg);
	    php_hermes_debug_server("0: an malformed request '%s'" TSRMLS_CC, payload), 
		efree(err_msg);
		goto response_no_output;
	}

	php_hermes_debug_server("%ld: accpect rpc request form '%s'" TSRMLS_CC,
			header->id, header->provider? (char *)header->provider : "Hermes PHP " HERMES_VERSION);

	if (!(post_data = php_hermes_packager_unpack(payload, payload_len, &err_msg))) {
        php_hermes_error(response, HERMES_ERR_PACKAGER, err_msg);
		efree(err_msg);
		goto response_no_output;
	}

	request = php_hermes_request_instance(post_data TSRMLS_CC);
	ce = Z_OBJCE_P(obj);

	if (!php_hermes_request_valid(request, response TSRMLS_CC)) {
		goto response_no_output;
	}

	if (php_start_ob_buffer(NULL, 0, 0 TSRMLS_CC) != SUCCESS) {
		php_hermes_error(response, HERMES_ERR_OUTPUT, "start output buffer failed");
		goto response_no_output;
	}

	ce = Z_OBJCE_P(obj);
	if (!zend_hash_exists(&ce->function_table, Z_STRVAL_P(request->method), Z_STRLEN_P(request->method) + 1)) {
		php_hermes_error(response, HERMES_ERR_REQUEST, "call to undefined api %s::%s()", ce->name, Z_STRVAL_P(request->method));
		goto response;
	}
	...
	zend_try {
		...
		if (zend_hash_exists(&ce->function_table, ZEND_STRS("_auth"))) {
			...
			if (call_user_function_ex(NULL, &obj, &func, &retval_ptr, 2, func_params, 0, NULL TSRMLS_CC) != SUCCESS) {
				...
				php_hermes_error(response, HERMES_ERR_REQUEST, "call to api %s::%s() failed", ce->name, Z_STRVAL(func));
				goto response;
			}
			...
			if (retval_ptr) {
               if (Z_TYPE_P(retval_ptr) == IS_BOOL && !Z_BVAL_P(retval_ptr)) {
				   ...
				   php_hermes_error(response, HERMES_ERR_REQUEST, "%s::_auth() return false", ce->name);
				   goto response;
			   }
			   zval_ptr_dtor(&retval_ptr);
			   retval_ptr = NULL;
		...
		if (call_user_function_ex(NULL, &obj, request->method, &retval_ptr, count, func_params, 0, NULL TSRMLS_CC) != SUCCESS) {
		    ...
		    php_hermes_error(response, HERMES_ERR_REQUEST, "call to api %s::%s() failed", ce->name, Z_STRVAL_P(request->method));
			goto response;
		}
		...
		if (retval_ptr) {
			php_hermes_response_set_retval(response, retval_ptr TSRMLS_CC);
		}
	} zend_catch {
		bailout = 1;
	} zend_end_try();
	...
	if (EG(exception)) {
		php_hermes_response_set_exception(response, EG(exception) TSRMLS_CC);
		EG(exception) = NULL;
	}
...
response:
	...
	php_hermes_response_alter_body(response, Z_STRVAL(output), Z_STRLEN(output), HERMES_RESPONSE_REPLACE TSRMLS_CC);

response_no_output:
	php_hermes_server_response(request, response TSRMLS_CC);
	if (request) {
		php_hermes_request_dtor(request TSRMLS_CC);
	}
	php_hermes_response_dtor(response TSRMLS_CC);
...
static void php_hermes_server_info(zval *obj TSRMLS_CC) /* {{{ */ {
	char buf[1024];
    ...
    zend_hash_apply_with_argument(&ce->function_table, (apply_func_arg_t)php_hermes_print_info, (void *)(ce) TSRMLS_CC);

    php_body_write(ZEND_STRS(HTML_MARKUP_FOOTER) - 1 TSRMLS_CC);
}
/* }}} */

/* {{{ proto Hermes_Server::__construct($obj, $protocol = NULL)
   initizing an Hermes_Server object */
PHP_METHOD(hermes_server, __construct) {
    zval *obj;
    ...
    zend_update_property(hermes_server_ce, getThis(), "_executor", sizeof("_executor")-1, obj TSRMLS_CC);
}
/* }}} */

/* {{{ proto Hermes_Server::handle()
   start service */
PHP_METHOD(hermes_server, handle)
{
	char *buf;
	int buf_len;
    ...
    if (SG(headers_sent)) {
		...
		executor = zend_read_property(hermes_server_ce, getThis(), ZEND_STRL("_executor"), 0 TSRMLS_CC);
		...
		if (!method || strncasecmp(method, "POST", 4)) {
			php_hermes_server_info(executor TSRMLS_CC);
            RETURN_TRUE;
		}

		php_hermes_server_handle(executor TSRMLS_CC);
	}
...
/* {{{ hermes_server_methods */
zend_function_entry hermes_server_methods[] = {
	PHP_ME(hermes_server, __construct, arginfo_service___construct, ZEND_ACC_PUBLIC|ZEND_ACC_CTOR|ZEND_ACC_FINAL)
	PHP_ME(hermes_server, handle, arginfo_service_void, ZEND_ACC_PUBLIC)
	PHP_FE_END
};
/* }}} */

HERMES_STARTUP_FUNCTION(service) /* {{{ */ {
    zend_class_entry ce;

    INIT_CLASS_ENTRY(ce, "Hermes_Server", hermes_server_methods);
    hermes_server_ce = zend_register_internal_class(&ce TSRMLS_CC);
	zend_declare_property_null(hermes_server_ce, ZEND_STRL("_executor"), ZEND_ACC_PROTECTED TSRMLS_CC);

    return SUCCESS;
}
/* }}} */

HERMES_SHUTDOWN_FUNCTION(service) /* {{{ */ {
	return SUCCESS;
} /* }}} */
